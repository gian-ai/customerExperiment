...
def customerImport(dbClient, rawData, platform:str, title:str|None=None, country:str|None=None)->dict:
    ...
    checkPhone = True if platform == "Phone" or platform == "Email" else False
    ...
    if platform=="LinkedIn":    
        ...
        cols = ["Email", "First Name", "Last Name", "Lead Source",  "Lead Status",
                 "Title", "Specialty", "Company"]

        if checkPhone:
            if "Phone" in rawCols: cols.append("Phone") 
            elif "Phone Number" in rawCols: cols.append("Phone Number") 
            else: checkPhone = False

        if "Product-of-Interest-" in rawCols: cols.append("Product-of-Interest-")
        if "Product of Interest" in rawCols: cols.append("Product of Interest")
        
        if "Country-" in rawCols: cols.append("Country-")  
        if "Country" in rawCols: cols.append("Country")
        if "Country List" in rawCols: cols.append("Country List")  
        ...
        if checkEmail:
            if "Email" in rawCols: columns.append("Email")
        clean = rawDF[columns]
...
def createVariable(dbClient, generatorID:int, painPoint:str, contentA:str|None=None, contentB:str|None=None, contentC:str|None=None, contentD:str|None=None, contentE:str|None=None,):
    ...
    if data:
        ...
        if not df.empty:
            # If it is not empty, then the variable generator already has variables.
            # If all of the variables are the same, then do not add it again.
            # If any of the variables are different, add it again.

            if contentA != None and not variableIsUnique:
                # if there are no rows in the check, it is unique.
            ...
            if variableIsUnique:  
                ...
                # However, there are variables that have the same content.
                return None
...
def generateExperiments(dbClient, ownerEmail, experimentGeneratorID) -> dict:
    if type(experimentGeneratorID) is not list:
        experimentGeneratorID = [experimentGeneratorID]
...
def getVariableGenerators(db, ownerEmail:str)->dict:
    get_data = db.collection("variableGenerators").where(filter=firestore.FieldFilter("ownerEmail","==",ownerEmail)).get()
